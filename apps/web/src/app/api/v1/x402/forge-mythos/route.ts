...
// Protegido por x402: paga $0.05 USDC → chama journey_orchestrator.forge_mythos()
// =============================================================================
...
const routeConfig = X402_ROUTES["POST /api/v1/x402/forge-mythos"];
...
async function handler(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { userAddress, tokenIds, fee } = body;

    if (!userAddress || !tokenIds || !Array.isArray(tokenIds) || tokenIds.length < 2) {
      return NextResponse.json(
        { error: "Missing required fields: userAddress, tokenIds (array, min 2), fee" },
        { status: 400 }
      );
    }
    ...
    // Monta a chamada forge_mythos(user, ids, fee)
    const sdk = await import("@stellar/stellar-sdk");
    const idsScVal = sdk.xdr.ScVal.scvVec(
      tokenIds.map((id: number) => nativeToScVal(id, { type: "u32" }))
    );
    ...
    const call = contract.call(
      ...
      idsScVal,
      nativeToScVal(BigInt(fee || 500_0000000), { type: "i128" })
    );
    ...
    return NextResponse.json({
      success: true,
      message: "Payment received. Forge transaction prepared for signing.",
